...
function promoteImageRemedy(
  ...
  const unused = selections
    ...
    //
    // BY PATH AS WELL AS BY SLIDE: one file can arrive on two selection rows,
    // so a picture excluded here by its archetype can still come back through
    // the other row and land on the cover anyway.
