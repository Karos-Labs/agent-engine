...
export function createInstagramAgentWorkflow(options: CreateInstagramAgentWorkflowOptions) {
  ...
  return async function instagramAgentWorkflow(wf: WorkflowContext): Promise<InstagramAgentWorkflowResult> {
      ...
      return planRunBudget(
        {
          ...
          // cannot know whether the STORY will earn a concept, so per
          // `run-budget.ts`'s own rule — an estimate that flatters itself pulls
          // no lever — this says only that the mode is not switched off
          // outright. What the estimator does with it is gated on the PLAN:
          // the $0.030 is booked only on rungs that keep `generatedImagesCap`
          // above zero and `optionalRevets` on, because the selector declines
          // outright on the rest and the spend is then provably impossible.
    ...
    const draftOnce = async (revision: number, notes: readonly RevisionNote[]): Promise<DraftResult> => {
      ...
      const conceptVerdict: ConceptEligibility = await wf.step.code(rev("04m-concept-eligibility"), () =>
        conceptEligibility({
      ...
      if (conceptVerdict.eligible) {
        ...
        spend(rev("04n-design-concept"), conceptExec.totalCostUsd, STEP_COST_ESTIMATES_USD.concept);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      // The native-language marker is attempt-scoped for the same reason and reset in the same place: an
      ...
      if (concept !== undefined) {
        const bound = await wf.step.code(rev(`04o-apply-concept-attempt-${attempt}`), () => {
