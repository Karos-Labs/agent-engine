...
/**
 * The deterministic script floor, and the table it reads.
 *
 * MOVED to `@agent-engine/workflow` on 2026-09-18 and re-exported here, so
 * every call site in this agent and every assertion in its tests is unchanged.
 * The table's own doc comment is the reason it moved rather than being copied:
 * "two tables drift, and a language the inference knows but the gate does not
 * (or vice versa) is a client whose gate silently never runs" — which is just
 * as true of a second agent as of a second module. The TikTok family needed
 * the same check and now shares this one.
 *
 * What stayed here is everything shaped like THIS agent's content: the text
 * both stages judge, the slide slot walkers, the native-editor axes.
 */
export {
  checkExpectedScript,
