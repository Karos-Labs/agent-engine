...
describe("P0 parity-audit Fix 4: image rights/watermark verification holds the whole post", () => {
  ...
  it("holds the whole post when a slide's selected image is rightsUsable: false, exactly like no candidate existing at all", async () => {
    ...
    const router = fakeRouterSequence([finalTurn(goodResearchOutput()), finalTurn(goodCopyOutput()), finalTurn(vetting)]);
    ...
    expect(result.reason).toMatch(/not rights-usable/i);
  ...
  it("holds the whole post when a slide's selected image is watermarkFree: false", async () => {
    const promptStore = makePromptStore();
    ...
    expect(result.status).toBe("held");
    if (result.status !== "held") throw new Error("unreachable");
    expect(result.reason).toMatch(/no viable image found for slide\(s\) 5/i);
    expect(result.reason).toMatch(/not watermark-free/i);
  });
});
...
describe("P0 parity-audit Fix 3: cross-post image-reuse prevention", () => {
  ...
  it("rejects a candidate whose path matches a prior post's already-used image, holding the whole post", async () => {
    const pool = goodImageCandidatePool();
    ...
    expect(result.status).toBe("held");
    if (result.status !== "held") throw new Error("unreachable");
    expect(result.reason).toMatch(/no viable image found for slide\(s\) 1/i);
    expect(result.reason).toMatch(/already used in a prior post/i);
  });
