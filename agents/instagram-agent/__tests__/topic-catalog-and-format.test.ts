...
function rankedWith(rows: Array<{ candidate: TrendCandidate; score: number }>): RankedTopics {
  return {
    ranked: rows.map((r) => ({
      ...r,
      components: { brandFit: r.candidate.brandFit, interest: r.candidate.interest, distance: 1, modeBonus: 1, engineBonus: 1, freshness: 1, engine: "niche-news", engineRotation: 1, subjectRotation: 1 as const },
    })),
