...
describe("zero-held guarantee: a picture problem never costs the post", () => {
  ...
  it("completes with every slide typographic when EVERY media tier is dead at once", async () => {
    ...
    const router = fakeRouterSequence([finalTurn(goodTrendScoutOutput()), finalTurn(goodResearchOutput()), finalTurn(goodAngleProposal()), finalTurn(copy), finalTurn(goodRelevanceVerdict()), finalTurn(goodVisualQaOutput())]);
  ...
  it("completes when the generative tier 429s on SOME slides, keeping the images it did get", async () => {
    ...
    const router = fakeRouterSequence([
      ...
      finalTurn({
        selections: [
          { n: 1, imagePath: genPath, reason: "the generated image matches", license: "Generated image", rightsUsable: true, watermarkFree: true, claimMatch: 5, claimMatchReason: "drawn to the slide's claim" },
        ],
      }),
  ...
  it("still blocks intake when the client has no config, because there is no client to write for", async () => {
    ...
    try {
      const router = fakeRouterSequence([finalTurn(goodTrendScoutOutput()), finalTurn(goodResearchOutput())]);
