...
describe("InstagramImageVettingAgent @6", () => {
  it("is pinned to the latest instagram-image-vet, and every frozen version still carries the rule the next one was built on", () => {
    ...
    // `image-vet-floor.test.ts` for the run that made it necessary). What this
    // test has always
    ...
    expect((agent as unknown as { config: { skillRef: string } }).config.skillRef).toBe("instagram-image-vet@10");
