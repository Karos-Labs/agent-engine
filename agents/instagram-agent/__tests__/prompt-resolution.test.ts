...
describe("PromptStore resolution (RFC-01 §16.1) — nothing here is a hardcoded prompt literal", () => {
  ...
  it("every instagram prompt this phase touched has a row in scripts/prompt-registry.ts", () => {
    ...
    expect(registry).toMatch(/promptId: "instagram-copy"[\s\S]{0,700}latestVersion: "39"/);
  ...
  it("every agent reads the version this phase shipped: copy @24, post package @2, visual QA @5, image vet @7, entities @1, art director @2, concept @1", async () => {
    ...
    expect((copy as unknown as { config: { skillRef: string } }).config.skillRef).toBe("instagram-copy@39");
  ...
  it("every pinned instagram prompt resolves, each byte-identical to its own latest.md", async () => {
    ...
    for (const [promptId, version, h1] of [
      ["instagram-copy", "39", "# Instagram Copy Craft Guide, v39"],
  ...
  it("instagram-copy/latest.md is BYTE-identical to 23.md, and 19.md is still exactly 18.md plus §29", () => {
    ...
    const v39 = readFileSync(path.join(PROMPTS_ROOT, "instagram-copy", "39.md"));
    expect(latest.equals(v39), "latest.md must be byte-identical to 39.md, not merely equivalent").toBe(true);
    ...
    // v39 (2026-09-26) adds the optional callouts paragraph to section 28 only.
    expect(v39.toString("utf8")).toContain("**Callouts: labels with an arrow into the picture (optional).**");
    expect(before28(v39)).toBe(before28(v38));
