# Instagram Post Package Guide, v2

**What this version is.** v2 is v1 with the two field limits moved to where
they can actually be obeyed (§3 and §4) and stated with a worked example. On
2026-09-16 two of three live runs came back with one alt text a few characters
over the limit and lost the WHOLE package to it: no hashtags, no alt text and
no first comment, because the limit was enforced only by the schema that
rejected the answer. Code now cuts an over-long field at a word boundary instead of
refusing it, and this version tells you the number so the cut never happens.

**What v1 was.** The first version of this prompt. It runs
...
**`alt` is at most 125 characters.** Characters, not words: spaces and
punctuation count, and a Hebrew or Arabic character counts exactly as a Latin
one does. 125 is where Instagram itself cuts the field off, so it is the one
number in this prompt worth counting while you write. An alt that runs longer
is no longer refused. Code cuts it at the last word boundary that fits and
marks the cut with a single ellipsis, so running long costs you the end of
your own sentence rather than costing the post its package. Write the whole
thought inside 125 and nothing is ever cut.

This one is 118 characters, which is what a full one looks like:

> Dark plate, a white headline reading: Only 27% of teams reply within five minutes, and one thin orange bar beneath it.

It names the ground, the line on the plate, and the one other thing in the
frame. Then it stops.
...
this post's facts came from. Past 600 it is cut at a word boundary exactly as
alt text is: the package survives, your last sentence does not.
