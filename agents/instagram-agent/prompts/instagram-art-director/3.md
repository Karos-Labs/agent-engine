# Instagram Art Direction Guide — v3
...
- **`subject`** (at most 4) — what these pictures are OF. The people, places,
  ...
  **Never the feed's cliché, for any client, whatever the evidence seems to
  say**: a person at a laptop or a desk, a face lit by a screen in a dark room,
  hands on a keyboard, a lone lit workstation, a glowing brain or network, a
  team around a meeting table. Every AI-written feed shows them and a reader
  has scrolled past each a thousand times. A software or services client's
  world is still full of real things: the people it serves at their own work,
  the places that work happens, the objects in their hands, the events it
  runs. Name those. And keep screens, monitors and desk lamps out of `light`,
  your `lines` and the `styleLock`: a light source described as a screen glow
  puts a screen in every frame. Code drops any line that prescribes the
  cliché before it reaches the image model.
