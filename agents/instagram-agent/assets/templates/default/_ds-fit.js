...
(function () {
  ...
  function overflows() {
    ...
    for (var g = 0; g < content.length; g++) {
      ...
      /* An INLINE box is measured by its face's ascent and descent, not its
         line box (see the limb below), so it pokes a few px past a field its
         block sits inside. Emphasis wraps every run of a marked field in an
         inline span (`.mk`/`.mk-t`), and once a one-colour brand's marks
         painted (2026-09-26) those spans made this limb shrink karoslabs'
         slide 3 two steps to caption size on a plate 5% covered by text. The
         block that holds them is measured here anyway. */
      var boxDisplay = getComputedStyle(content[g]).display;
      if (boxDisplay === 'inline' || boxDisplay === 'contents') continue;
  ...
  function pass() {
    ...
    /* NOT OVER A FULL-BLEED PHOTOGRAPH (2026-09-25). On a poster the grown
       title climbs the frame, the shade follows it up (`anchorScrim`), and the
       picture under it is gone: prep batch 6's karoslabs cover set five display
       lines from a third of the way down and its photograph of a phone showing
       ChatGPT printed as near-black. The title keeps its role's step there, so
       the upper frame is the picture. */
    var overPhoto = document.querySelector('img.hero[src]:not([src=""]):not([data-kind]), img.hero[src]:not([src=""])[data-kind=""], .bg[data-kind=""] > img[src]:not([src=""]), .bg:not([data-kind]) > img[src]:not([src=""])') !== null
      && getComputedStyle(document.querySelector('.scrim') || document.body).display !== 'none';
    /* A sandwich cover's picture is the space BETWEEN the title and the deck;
       a grown title would eat it. */
    var sandwich = document.querySelector('img.hero[data-kind="sandwich"]') !== null;
    if (primary.length && !overPhoto && !sandwich) {
    ...
    /* THE HEADLINE TAKES ITS STEP BACK WHEN IT FITS. The set shrank together
       so the relationship held; if the plate now fits with the primary one
       step LARGER than the rest, it keeps that step. A first-position list
       whose rows and title both fell a step measured 24% of air above the
       title (calibration, 2026-09-25); the title back at its size fills it,
       and the gap between title and rows widens instead of closing. */
    if (step > 0 && primary.length) {
      set(primary, step - 1);
      closeInk();
      if (overflows()) { set(primary, step); closeInk(); }
    }
  ...
  /* THE SANDWICH (2026-09-26): the picture fills exactly the space between the
     title's last line and the deck's first, a gap apart from each. Measured
     after the type is set, like `fillPicture`. */
  function placeSandwich() {
    var pic = document.querySelector('img.hero[data-kind="sandwich"]');
    if (!pic) return;
    var title = plate.querySelector('#title');
    var deck = plate.querySelector('.lockup > .body-text');
    if (!title) return;
    var gap = 32;
    var titleRect = title.getBoundingClientRect();
    var deckTop = deck && deck.getBoundingClientRect().height > 0 ? deck.getBoundingClientRect().top : plate.getBoundingClientRect().bottom - parseFloat(getComputedStyle(plate).paddingBottom || '0');
    var top = Math.round(titleRect.bottom + gap);
    var height = Math.round(deckTop - gap - top);
    if (height < 240) return;
    document.body.style.setProperty('--sandwich-top', top + 'px');
    document.body.style.setProperty('--sandwich-h', height + 'px');
    document.body.setAttribute('data-sandwich', String(height));
  }
  function resetPicture() {
    document.body.style.removeProperty('--frame-h');
    document.body.style.removeProperty('--inset-h');
    document.body.style.removeProperty('--sandwich-top');
    document.body.style.removeProperty('--sandwich-h');
