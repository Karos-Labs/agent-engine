...
describe("Instagram agent golden runs — deterministic assertions (RFC-01 §12)", () => {
  ...
  for (const goldenRun of INSTAGRAM_GOLDEN_RUNS) {
    it(`${goldenRun.id}: the endorsed research/copy/selections triple passes step 07's real self-check`, () => {
      const results = runInstagramDeterministicAssertions(goldenRun);
    ...
    it(`${goldenRun.id}: is deterministic — running it twice gives identical verdicts`, () => {
      const first = runInstagramDeterministicAssertions(goldenRun);
      const second = runInstagramDeterministicAssertions(goldenRun);
      expect(first).toEqual(second);
    });
  }

  it("catches a regression: a slide whose sourceRef no longer traces to a real fact fails the self-check", () => {
    ...
    const results = runInstagramDeterministicAssertions(regressed);
  ...
  it("catches a regression: reintroducing a banned word into slide copy fails the self-check", () => {
    const goldenRun = INSTAGRAM_GOLDEN_RUNS[0]!;
    const regressed = {
      ...goldenRun,
      endorsedCopy: {
  ...
  it("catches a regression: an unfillable slide (null imagePath) fails the no-unfillable-slide check", () => {
    const goldenRun = INSTAGRAM_GOLDEN_RUNS[0]!;
    const regressed = {
      ...goldenRun,
      endorsedSelections: {
  ...
  it("catches a regression: canvas.scale drifting away from 2 fails the canvas.scale check", () => {
    const goldenRun = INSTAGRAM_GOLDEN_RUNS[0]!;
