...
describe("checkpoint resume idempotency (RFC-01 §8.1)", () => {
  ...
  it("re-running engine.run() with the same runId does not re-execute any already-completed step", async () => {
    ...
    expect(stepRecords).toHaveLength(30) /* +1: 15r-repair-edition */; // AU20 added the verified-dedupe step; 2026-09-05 added the research plan, edition plan, editorial lint, editor verdict and email render
  ...
  it("resumes correctly after a mid-run crash: earlier steps aren't redone, the run still reaches completed", async () => {
    ...
    expect(finalSteps).toHaveLength(30) /* +1: 15r-repair-edition */; // AU20 added the verified-dedupe step; 2026-09-05 added the research plan, edition plan, editorial lint, editor verdict and email render
