...
export interface RenderPreviewResult {
  ...
  wordCount: number;
  /** True only when the title, meta description, and body all clear their own limits. */
  withinLimit: boolean;
 ...
 * `gate.brandCompliance`), which only ever see the composed `text`. Lives
 * alongside the agent that uses it rather than in `packages/tools/`, since
 * it's blog-specific rendering logic, not a general Layer 3 capability
 * every agent needs.
 */
export const renderPreview: AgentTool<RenderPreviewInput, RenderPreviewResult> = defineTool({
  ...
  async execute({ title, metaDescription, text }) {
    ...
    return success<RenderPreviewResult>({
      ...
      bodyWithinLimit,
      wordCount,
      withinLimit: titleWithinLimit && metaDescriptionWithinLimit && bodyWithinLimit,
      aboveTheFold,
      rendered,
    });
  },
