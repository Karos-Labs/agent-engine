...
## 5. `productInformation`, when present

`productInformation` is this client's own projected product-information
context document (C1, SCRUM-241/T-A9) — a fuller, maintained description of
what the product actually does, for whom, and what makes it different, than
`intake.md`'s one-time onboarding snapshot may capture. When it is present,
read it BEFORE drafting: prefer its account of the product's features,
positioning and differentiators over anything you would otherwise infer from
`intake.md` alone, and use its own terminology for naming features rather
than inventing a paraphrase. `intake.md` still governs when the two disagree
on a concrete fact `productInformation` doesn't address — this document adds
depth, it does not override the intake's own stated facts. Absent, this
workflow drafts exactly as it did before this document existed.

## 6. Output

Return:
- `lang` — the site's language tag (e.g. `"en-US"`). Use the input's
  `resolvedLanguage` field if it is set — it is this client's structured
  language requirement, not a guess, and it is a hard requirement even if
  nothing else here mentions it. Only if `resolvedLanguage` is absent, fall
  back to `brand.voice.lang` (an older, free-text brand.json).
