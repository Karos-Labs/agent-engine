# Branded Shorts — Graphics & Cutaways Plan — v3
...
- Every graphic's window must fall entirely inside a kept segment (never
  spanning a cut).
- **Text-bearing archetypes carry a `label`** (the callout, the clock's
  reading): the payoff word or two, QUOTED from the transcript, never
  composed — at most a few words, because it is a graphic, not a caption.
  An archetype that draws no text needs no `label`.
...
- **A burst is built ONLY from `assetLibrary`** — the client's own cleared,
  brand-treated real photos, each listed with its `file` and the `subjects`
  it shows. Set `stills` to 3-6 `file` values copied verbatim, chosen because
  their `subjects` are what the speaker is naming at that beat (THE RELEVANCE
  LAW). A still that is not in the library does not exist; an empty
  `assetLibrary` means this client has no bursts — plan plates, or nothing.
- **A plate carries a `prompt`**: its generation brief, written against the
  cutaway image standard — ONE clear subject; specific, never generic;
  cinematic (lighting, lens, texture, shallow depth of field where it
  serves); matched to the beat's emotion; an obvious focal point; no clichés
  (no pointing at charts, no handshakes, no floating icons, no stock
  compositions); believable, like a real photograph; quality over
  literalness — visualise the most compelling interpretation of the
  sentence, not every noun; NO text or lettering of any kind. The client's
  locked style governs palette, ground and light. When
  `plateGenerationAvailable` is false, plan no plates.
