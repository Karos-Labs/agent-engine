...
describe("checkpoint resume idempotency (RFC-01 §8.1)", () => {
  ...
  it("re-running engine.run() with the same runId does not re-execute any already-completed step", async () => {
    ...
    expect(stepRecords).toHaveLength(29); // AU20 added 10a-verify-not-duplicate; C7 added 01b-read-learning-context and 21-write-run-state
  ...
  it("resumes correctly after a mid-run crash: earlier steps aren't redone, the run still reaches completed", async () => {
    ...
    expect(finalSteps).toHaveLength(29); // AU20 added 10a-verify-not-duplicate; C7 added 01b-read-learning-context and 21-write-run-state
