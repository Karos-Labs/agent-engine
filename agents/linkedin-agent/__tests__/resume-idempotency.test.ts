...
describe("checkpoint resume idempotency (RFC-01 §8.1)", () => {
  ...
  it("re-running engine.run() with the same runId does not re-execute any already-completed step", async () => {
    ...
    expect(stepRecords).toHaveLength(31); // AU20 added the verified-dedupe step; C7 added 01b + 19; D22 added 12b; linkedin-craft@8 added 12a-verify-dated-language
  ...
  it("resumes correctly after a mid-run crash: earlier steps aren't redone, the run still reaches completed", async () => {
    ...
    expect(finalSteps).toHaveLength(31); // AU20 added the verified-dedupe step; C7 added 01b + 19; D22 added 12b; linkedin-craft@8 added 12a-verify-dated-language
