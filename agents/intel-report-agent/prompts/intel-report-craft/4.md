...
etc.) has no field in this schema yet — §3's `targetAudience` document, when
present, is an INPUT that steers judgment inside the existing sections, not
a new output field, and does not close this gap. Do not attempt to smuggle a
structured ICP blueprint into another field; it's a real, acknowledged
omission for a future revision, not something to work around here.
