...
/* The signature set-piece: a scroll-drawn progress graph. The home for a
   client's carry-forward dashboard chart, rebuilt to the new brand and tokens.
   The line draws as you scroll into it. Unit + delta label are content-driven. */
...
export function SignatureShowcase({ data }: { data: SignatureShowcaseContent }) {
  ...
  return (
    <section id={data.id ?? "progress"} className="border-t border-edge">
          ...
          <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
            <div>
              <div className="flex items-baseline gap-2">
                ...
                {data.unit && <span className="font-mono text-sm text-fg-2">{data.unit}</span>}
              </div>
