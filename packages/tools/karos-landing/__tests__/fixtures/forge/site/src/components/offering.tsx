    ...
    <section id="pricing" className="mx-auto max-w-6xl px-5 py-28 sm:px-8">
      ...
      <RevealGroup className="mt-12 grid gap-6 lg:grid-cols-3" stagger={0.1}>
        {data.plans.map((p) => {
          ...
          const isFree = base === 0;
          const shownPrice = isFree ? p.price : annual ? `$${Math.round(base * 0.83)}` : p.price;
          const shownCadence = isFree ? p.cadence : "/mo";
          return (
            <RevealItem key={p.name}>
              ...
              >
                ...
                <div className="mt-4 flex items-baseline gap-1">
                  <span className="font-display text-5xl font-extrabold text-fg">{shownPrice}</span>
                  <span className="font-mono text-sm text-fg-2">{shownCadence}</span>
                </div>
