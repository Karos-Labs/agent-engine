...
export interface HarvestPodcastResult {
  ...
  provider: string;
  discovery: "allowlist" | "open";
  /** Always `"audio"`. Explicit so a caller cannot mistake this for the video harvest's output. */
  media: "audio";
}

/** Audio containers a podcast feed actually serves. Refused, never guessed: a feed that hands back HTML has not handed back an episode. */
const ACCEPTED_AUDIO_MIME: Record<string, string> = {
  ...
  "audio/wav": ".wav",
  "audio/x-wav": ".wav",
  // Some feeds serve a video enclosure. The pipeline transcribes either.
  "video/mp4": ".mp4",
  "video/quicktime": ".mov",
};
  ...
  return defineTool({
    ...
    async execute(input, { ctx: _ctx }) {
      ...
      for (const episode of ranked) {
        ...
        const extension = ACCEPTED_AUDIO_MIME[mime];
        ...
        const fileName = `${OUTPUT_STEM}${extension}`;
