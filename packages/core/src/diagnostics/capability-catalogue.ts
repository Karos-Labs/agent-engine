...
export const CAPABILITY_CATALOGUE: readonly CapabilityDefinition[] = [
  ...
  },
  {
    ...
    requires: [
      ...
      { name: "SCRAPPYCOCO_API_KEY", kind: "enhances" },
    ],
    whenAbsent:
      "Each engine's cells report UNAVAILABLE/no_adapter_wired, honestly, exactly like every other unconfigured engine — never a fabricated MEASURED/ESTIMATED answer. Claude's capture independently depends on ANTHROPIC_API_KEY, a row elsewhere in this catalogue for another capability, rather than a new credential. GEMINI_API_KEY is carried on THIS row rather than reused from a model-routing row: AU59/SCRUM-358 removed the direct-Gemini model route and its row, so capture is now this variable's only reader — and absent it, Gemini falls back to the Vertex route on ADC, which needs no credential of its own and so has no entry here. OPENAI_API_KEY backs the ChatGPT column via the Responses API's web_search tool by default. SCRAPPYCOCO_API_KEY (its own row elsewhere, for research.pull's scraper) ALSO backs capture as of 2026-09-20: `copilot` (which had no route at all before this — see scrappycoco-answer-engine.ts) and `aimode` wire from this key alone, and it can back `chatgpt` instead of OpenAI's route when AI_VISIBILITY_CHATGPT_SOURCE=scrappycoco is set, meant for comparing the two before picking one rather than as a silent fallback. `google_aio` (a different Google product from `aimode`) still has no working route: the capability's own description claims a `web.search_web` parameter pair for it that the account's live schema rejects — verified, not assumed.",
